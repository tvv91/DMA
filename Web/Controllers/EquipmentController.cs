...
namespace Web.Controllers
{
    public class EquipmentController : Controller
    {
        ...
        {
            return Ok(await _repository.States.Where(x => x.Data.Contains(term)).Select(x => new AutocompleteResponse { Label = x.Data }).ToArrayAsync());
        }

        [HttpGet("search/codec")]
        public async Task<IActionResult> SearchCodec(string term)
        {
            return Ok(await _repository.Codecs.Where(x => x.Data.Contains(term)).Select(x => new AutocompleteResponse { Label = x.Data }).ToArrayAsync());
        }
        ...
        {
            return Ok(await _repository.Formats.Where(x => x.Data.Contains(term)).Select(x => new AutocompleteResponse { Label = x.Data }).ToArrayAsync());
        }

        [HttpGet("search/device")]
        public async Task<IActionResult> SearchDevice(string term)
        {
            return Ok(await _repository.Devices.Where(x => x.Data.Contains(term)).Select(x => new AutocompleteResponse { Label = x.Data }).ToArrayAsync());
        }
