...
namespace Web.Db.Implementation
{
    public class StatisticRepository : IStatisticRepository
    {
        ...
        {
            var result = await _context.Statistic.FirstOrDefaultAsync();

            if (result == null)
            {
                result = await RefreshStatistic();
                await _context.Statistic.AddAsync(result);
            ...
            else
            {
                ...
                {
                    result = await RefreshStatistic();
                    await _context.Statistic
                        .Where(x => x.Id == 1)
                        .ExecuteUpdateAsync(x => x.SetProperty(e => e.Data, result.Data).SetProperty(e => e.LastUpdate, result.LastUpdate));
                }
            }
        ...
        private async Task<Statistic> RefreshStatistic()
        {
            ...
            {
                Data = JsonSerializer.Serialize(statisticData),
