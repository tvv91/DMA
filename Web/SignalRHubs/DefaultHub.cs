    ...
    public class DefaultHub : Hub
    {
        ...
        private async Task<List<EquipmentResponse>> GetWireItems(int page)
        {
            return await _techInfoRepository.Wires
                .Skip((page - 1) * ITEMS_PER_PAGE)
                .Take(ITEMS_PER_PAGE)
                .Include(x => x.WireManufacturer)
                .Select(x => new EquipmentResponse()
                {
                    Id = x.Id,
                    Model = x.Data,
                    Manufacturer = x.WireManufacturer.Data,
                }).ToListAsync();
        }
        ...
        public async Task GetTechnicalInfoIcons(string connectionId, int albumId)
        {
            ...
            else
            {
                ...
                {
                    await Clients.Client(connectionId).SendAsync("ReceivedTechnicalInfoIcon", "device", _imgService.GetImageUrl(tInfo.PlayerId.Value, EntityType.Player));
                }
